...
fault recovery:  five consecutive healthy samples at 20 Hz (250 ms)
```
...
5. Hold the healthy state. The fault must remain asserted for the first four 20 Hz samples and clear on the fifth.
6. Remove the healthy state. The fault must assert on the next sample (50 ms maximum task period, excluding scheduling jitter).
...
after the interface schematic is reviewed, assembled, electrically measured, fault-injected, and documented. The acknowledgement is a build interlock, not an automated proof.
